...
require "datamapper"
require "uuidtools"
require "dalli"  # Memcached client library
...
class VCAP::Services::Memcached::Node
  ...
  def restore(instance_id, backup_dir)
    ...
    if File.exists?(dump_file)
      if File.new(dump_file).size > 0
        ...
        memcached = Memcached.new({:port => instance.port, :password => instance.password})
        memcached.flushall
      end
    else
  ...
  def dump_instance(service_credentials, binding_credentials_list = [], dump_dir)
    ...
    # This will activate the memcached instance to do snapshot in 1 second.
  ...
  def destroy_instance(instance)
    raise MemcachedError.new(MemcachedError::MEMCACHED_DESTORY_INSTANCE_FAILED, instance.inspect) unless instance.destroy
  end
  ...
  def start_instance(instance, db_file = nil)
    ...
    if pid
      ...
      swap_file = File.join(dir, "memcached.swap")
  ...
  def stop_memcached_server(instance)
    ...
    begin
      ...
      if e.class == Errno::ECONNREFUSED
        ...
        # should be a memcached ruby binding bug. Here just ignore it.
      elsif e.class == RuntimeError
        # It could be a disabled instance
        memcached = Memcached.new({:port => instance.port, :password => @disable_password})
        begin
          memcached.shutdown
        rescue => e
        end
  ...
  def check_password(port, password)
    memcached = Memcached.new({:port => port})
    ...
    begin
      memcached.quit if memcached
    rescue => e
    end
  end

  def get_info(port, password)
    memcached = Memcached.new({:port => port, :password => password})
    memcached.info
  rescue => e
    raise MemcachedError.new(MemcachedError::MEMCACHED_CONNECT_INSTANCE_FAILED)
  ensure
